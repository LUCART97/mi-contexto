...
// Expone la herramienta `consultar_leads`, que lee datos reales de la tabla
// public.leads en Supabase (vía Management API, que no depende de RLS).
...
Deno.serve(async (req) => {
  ...
  if (method === "initialize") {
    return Response.json(
      jsonRpcResult(id, {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: "ia-vertyx-mcp", version: "1.0.0" },
      })
    );
  }
  ...
  if (method === "tools/call") {
    ...
    const args = (params?.arguments as { estado?: string } | undefined) ?? {};
    ...
    try {
      ...
      return Response.json(
        jsonRpcResult(id, {
          content: [{ type: "text", text: `Error consultando leads: ${err}` }],
          isError: true,
        })
      );
    }
